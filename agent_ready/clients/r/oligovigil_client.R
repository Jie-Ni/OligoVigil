...
oligovigil_search <- function(query, limit = 20, base_url = "http://127.0.0.1:8077") {
  oligovigil_json(base_url, "/api/search", list(q = query, limit = limit))
}

oligovigil_evidence_records <- function(domain = "", query = "", limit = 100, base_url = "http://127.0.0.1:8077") {
  oligovigil_json(base_url, "/api/evidence_records", list(domain = domain, q = query, limit = limit))
}

oligovigil_evidence_detail <- function(domain, evidence_id, base_url = "http://127.0.0.1:8077") {
...
oligovigil_safety_dossier <- function(sequence = "", helm = "", target = "", modification = "", delivery = "",
                                      endpoint = "", species = "", base_url = "http://127.0.0.1:8077") {
  oligovigil_json(
    base_url,
    "/api/safety_dossier",
    list(
...
oligovigil_benchmark <- function(base_url = "http://127.0.0.1:8077") {
  oligovigil_json(base_url, "/api/benchmark")
}

oligovigil_download_manifest <- function(base_url = "http://127.0.0.1:8077") {
  oligovigil_json(base_url, "/api/download_manifest")
}

oligovigil_offtarget_taxonomy <- function(base_url = "http://127.0.0.1:8077") {
  oligovigil_json(base_url, "/api/offtarget_taxonomy")
}

oligovigil_evidence_release <- function(base_url = "http://127.0.0.1:8077") {
  utils::read.csv(oligovigil_url(base_url, "/api/download/evidence_release.csv"), stringsAsFactors = FALSE)
}

oligovigil_benchmark_splits <- function(base_url = "http://127.0.0.1:8077") {
  utils::read.csv(oligovigil_url(base_url, "/api/download/benchmark_reference_splits.csv"), stringsAsFactors = FALSE)
}

oligovigil_benchmark_baselines <- function(base_url = "http://127.0.0.1:8077") {
  utils::read.csv(oligovigil_url(base_url, "/api/download/benchmark_baseline_results.csv"), stringsAsFactors = FALSE)
}

oligovigil_benchmark_tasks <- function(base_url = "http://127.0.0.1:8077") {
